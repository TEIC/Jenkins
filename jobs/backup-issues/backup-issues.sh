...
# MDH 2018-01-21: borrowed and modified this script from 
# for issue https://github.com/TEIC/TEI/issues/1714.
...
GHBU_ORG=${GHBU_ORG-"TEIC"}                                   # the GitHub organization whose repos will be backed up
...
# MDH: We don't use username and password
...
REPOLIST=`check curl --silent ${GHBU_API}/orgs/${GHBU_ORG}/repos\?per_page=100 -q | check grep "\"name\"" | check awk -F': "' '{print $2}' | check sed -e 's/",//g'`
...
# MDH: We don't need the code repo.
...
# MDH: We do need the issues, but without username and password.
   $GHBU_SILENT || echo "Backing up ${GHBU_ORG}/${REPO} issues"
   $GHBU_SILENT || echo "Backing up ${GHBU_ORG}/${REPO} issues"
   check curl --silent ${GHBU_API}/repos/${GHBU_ORG}/${REPO}/issues -q > ${GHBU_BACKUP_DIR}/${GHBU_ORG}-${REPO}.issues-${TSTAMP} && tgz ${GHBU_BACKUP_DIR}/${GHBU_ORG}-${REPO}.issues-${TSTAMP}
